...
process kleborate {
    publishDir "${OUTDIR}", mode: "${params.publish_mode}", overwrite: OVERWRITE, pattern: "results/${sample}-kleborate.txt}"
    tag "${sample}"
    ...
    output:
    file("results/$sample}-kleborate.txt") into MERGE_KLEBORATE

    shell:
    sample = fasta.getBaseName()
    resistance = params.skip_resistance ? "" : "--resistance"
    kaptive = params.skip_kaptive ? "" : "--kaptive"
    force_index = params.force_index ? "--force_index" : ""
    """
    mkdir results/
    kleborate -a !{fasta} \
        -o !{sample}-kleborate.txt !{resistance} !{kaptive} !{force_index} \
...
process merge_kleborate {
    publishDir "${OUTDIR}", mode: "${params.publish_mode}", overwrite: OVERWRITE, pattern: "kleborate-results.txt"
    beforeScript 'ulimit -Ss unlimited'

    input:
    file(tsv) from MERGE_KLEBORATE.collect()

    output:
    file("kleborate-results.txt")

    shell:
    """
    mkdir results/
    ls *-kleborate.txt | head -n1 | xargs -I {} head -n1 {} > results/kleborate-results.txt
    grep -v -P "strain\tspecies" *-kleborate.txt >> results/kleborate-results.txt
    mv results/kleborate-results.txt ./
    """
}
...
def print_help() {
    ...
    Kleborate Parameters:
        --skip_resistance       Turn off resistance genes screening
                                    Default: Screen resistance genes

        --skip_kaptive             Turn off Kaptive screening of K and O loci 
                                    Default: Run Kaptive for K and O loci

        --min_identity FLOAT    Minimum alignment percent identity for main results
                                    Default: ${params.min_identity}

        --min_coverage FLOAT    Minimum alignment percent coverage for main results 
                                    Default: ${params.min_coverage}

        --min_spurious_identity FLOAT
                                Minimum alignment percent identity for spurious results
                                    Default: ${params.min_spurious_identity}

        --min_spurious_coverage FLOAT
                                Minimum alignment percent coverage for spurious results 
                                    Deault: ${params.min_spurious_coverage}

        --min_kaptive_confidence STR
                                Minimum Kaptive confidence to call K/O loci - confidence levels 
                                    below this will be reported as unknown 
                                    Choices: None, Low, Good, High, Very_high, Perfect
                                    Default: ${params.min_kaptive_confidence}

        --force_index           Rebuild the BLAST index at the start of execution 
                                    Default: only build BLAST indices when they are missing
