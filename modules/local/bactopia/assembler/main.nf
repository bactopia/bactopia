...
conda_tools    = "bioconda::bactopia-assembler=1.0.0"
...
process ASSEMBLER {
    ...
    container "${ workflow.containerEngine == 'singularity' && !params.singularity_pull_docker_container ?
        'https://depot.galaxyproject.org/singularity/bactopia-assembler:1.0.0--hdfd78af_0' :
        'quay.io/biocontainers/bactopia-assembler:1.0.0--hdfd78af_0' }"

    input:
    tuple val(meta), path(fq), path(extra), path(genome_size)

    output:
    tuple val(meta), path(genome_size), path("results/${prefix}.{fna,fna.gz}"), file("total_contigs_*"), emit: fna, optional: true
    tuple val(meta), path("results/${prefix}.{fna,fna.gz}"), emit: fna_only, optional: true
    tuple val(meta), path("results/${prefix}.{fna,fna.gz}"), path(fq), emit: fna_fastq, optional: true
    path "results/*"
    path "${prefix}-assembly-error.txt", optional: true
    ...
    if [ "${use_original_assembly}" == "true" ]; then
        mkdir \${OUTDIR}
        gzip -cd ${extra} > \${OUTDIR}/${prefix}.fna
    ...
    if [ "\${TOTAL_CONTIGS}" -gt 0 ]; then
        assembly-scan \${OUTDIR}/${prefix}.fna > \${OUTDIR}/${prefix}.json
        TOTAL_CONTIG_SIZE=\$(grep "total_contig_length" \${OUTDIR}/${prefix}.json | sed -r 's/.*: ([0-9]+),/\\1/')
        if [ "\${TOTAL_CONTIG_SIZE}" -lt ${params.min_genome_size} ]; then
            mv \${OUTDIR}/${prefix}.fna \${OUTDIR}/${prefix}-error.fna
            mv \${OUTDIR}/${prefix}.json \${OUTDIR}/${prefix}-error.json
