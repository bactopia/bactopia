...
# Wrapper script to locate and run bactopia-citations.
# Search order:
#   1. bactopia-citations on PATH
...
# 1. Check PATH
if command -v bactopia-citations &>/dev/null; then
    exec bactopia-citations "$@"
fi

# 2. Search conda envs
if env_path=$(find_conda_env); then
    exec conda run --prefix "$env_path" bactopia-citations "$@"
fi

# 3. Give up
echo "ERROR: bactopia-citations not found." >&2
