...
# Wrapper script to locate and run bactopia-merge-schemas.
# Search order:
#   1. bactopia-merge-schemas on PATH
...
# 1. Check PATH
if command -v bactopia-merge-schemas &>/dev/null; then
    exec bactopia-merge-schemas "$@"
fi

# 2. Search conda envs
if env_path=$(find_conda_env); then
    exec conda run --prefix "$env_path" bactopia-merge-schemas "$@"
fi

# 3. Give up
echo "ERROR: bactopia-merge-schemas not found." >&2
