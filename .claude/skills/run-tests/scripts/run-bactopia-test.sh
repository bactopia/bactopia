...
# Wrapper script to locate and run bactopia-test.
# Search order:
#   1. bactopia-test on PATH
...
# 1. Check PATH
if command -v bactopia-test &>/dev/null; then
    exec bactopia-test "$@"
fi

# 2. Search conda envs
if env_path=$(find_conda_env); then
    exec conda run --prefix "$env_path" bactopia-test "$@"
fi

# 3. Give up
echo "ERROR: bactopia-test not found." >&2
