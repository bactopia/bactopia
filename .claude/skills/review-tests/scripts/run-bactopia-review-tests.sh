...
# Wrapper script to locate and run bactopia-review-tests.
# Search order:
#   1. bactopia-review-tests on PATH
...
# 1. Check PATH
if command -v bactopia-review-tests &>/dev/null; then
    exec bactopia-review-tests "$@"
fi

# 2. Search conda envs
if env_path=$(find_conda_env); then
    exec conda run --prefix "$env_path" bactopia-review-tests "$@"
fi

# 3. Give up
echo "ERROR: bactopia-review-tests not found." >&2
