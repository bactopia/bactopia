...
if [ "${IS_MAC}" == "1" ]; then
    ...
    update_environment "call_variants" "snippy=4.6.0 vcf-annotator=0.7 mash=2.3 ncbi-genome-download=0.3.1 biopython=1.77 snpeff=5.1 rename" ${CONDA_DIR} ${IS_MAC}
else
    ...
    update_environment "call_variants" "snippy=4.6.0 vcf-annotator=0.7 vt=2015.11.10=he941832_3 mash=2.3 ncbi-genome-download=0.3.1 biopython=1.77 snpeff=5.1 rename" ${CONDA_DIR} ${IS_MAC}
fi
update_environment "gather_samples" "fastp=0.23.2 sra-tools=3.0.1 bbmap=39.01 art=2016.06.05 mash=2.3 ncbi-genome-download=0.3.1 fastq-dl=1.1.1 fastq-scan=1.0.1 fastqc=0.11.9 lighter=1.1.2 porechop=0.2.4 nanoq=0.9.0 nanoplot=1.40.2 rasusa=0.7.0 biopython=1.77 rename" ${CONDA_DIR} ${IS_MAC}
