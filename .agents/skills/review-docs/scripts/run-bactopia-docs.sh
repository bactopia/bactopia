...
# Wrapper script to locate and run bactopia-docs.
# Search order:
#   1. bactopia-docs on PATH
...
# 1. Check PATH
if command -v bactopia-docs &>/dev/null; then
    exec bactopia-docs "$@"
fi

# 2. Search conda envs
if env_path=$(find_conda_env); then
    exec conda run --prefix "$env_path" bactopia-docs "$@"
fi

# 3. Give up
echo "ERROR: bactopia-docs not found." >&2
