...
- **Ground-truth assertions (D1xx)** — claims about repo state that are derivable from the live tree and SHOULD match what the docs say:
    ...
    - **D107** — skill inventory in [reference/06-skills.md](../../docs/reference/06-skills.md) must match `.agents/skills/*/SKILL.md`. Catches: skills on disk not listed in the table, rows referencing nonexistent skill directories, and purpose-cell drift from the SKILL.md `description:` first sentence.
...
- **Replace with current term**: most common case. `flattenPaths` → describe the current direct-emit pattern. `4-channel` → `2 channels (sample_outputs + run_outputs)`. `Tuple<Map, Path>` → `Channel<Record>`. Read the surrounding paragraph before editing — sometimes the whole sentence needs rewriting, not just a token swap. Look at [reference/01-examples.md](.agents/docs/reference/01-examples.md) for the modern equivalents.
