...
```
bash .agents/skills/review-groovydoc/scripts/run-bactopia-lint.sh --bactopia-path $BP --json --silent
```
...
```
python3 .agents/skills/release-checklist/scripts/release_audit.py --bactopia-path $BP --json
```
...
```
bash .agents/skills/merge-schemas/scripts/run-bactopia-merge-schemas.sh \
...
```
bash .agents/skills/update-catalog/scripts/run-bactopia-catalog.sh \
...
### 6. Docs sync (.agents/docs)

```
bash .agents/skills/review-docs/scripts/run-bactopia-docs.sh --bactopia-path $BP --validate --json --silent
```
...
```
bash .agents/skills/review-citations/scripts/run-bactopia-citations.sh --bactopia-path $BP --validate --json --silent
```
...
bash .agents/skills/review-tests/scripts/run-bactopia-review-tests.sh --bactopia-path $BP --run "$LATEST_RUN" --silent
```
