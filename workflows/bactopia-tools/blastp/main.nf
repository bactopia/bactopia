...
workflow {
    main:
    BACTOPIATOOL_INIT()
    BLASTP(BACTOPIATOOL_INIT.out.proteins, params.blastp_query)
    ch_sample_nf_logs = BLASTP.out.sample_outputs.flatMap { r -> r.nf_logs.collect { f -> tuple(r.meta, f) } }
    ch_run_nf_logs = BLASTP.out.run_outputs.flatMap { r -> r.nf_logs.collect { f -> tuple(r.meta, f) } }
    publish:
