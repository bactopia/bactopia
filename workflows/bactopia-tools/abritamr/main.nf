...
workflow {
    ...
    BACTOPIATOOL_INIT()
    ABRITAMR(formatSamples(BACTOPIATOOL_INIT.out.samples, BACTOPIATOOL_INIT.out.data_types))

    // Collect outputs
    ch_results = ch_results.mix(ABRITAMR.out.results)
    ch_logs = ch_logs.mix(ABRITAMR.out.logs)
    ch_nf_logs = ch_nf_logs.mix(ABRITAMR.out.nf_logs)
