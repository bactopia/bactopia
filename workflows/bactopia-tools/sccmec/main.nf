...
workflow {
    ...
    BACTOPIATOOL_INIT()
    SCCMEC(formatSamples(BACTOPIATOOL_INIT.out.samples, BACTOPIATOOL_INIT.out.data_types))

    // Collect outputs
    ch_results = ch_results.mix(SCCMEC.out.results)
    ch_logs = ch_logs.mix(SCCMEC.out.logs)
    ch_nf_logs = ch_nf_logs.mix(SCCMEC.out.nf_logs)
