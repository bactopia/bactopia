...
workflow {
    ...
    BACTOPIATOOL_INIT()
    PNEUMOCAT(formatSamples(BACTOPIATOOL_INIT.out.samples, BACTOPIATOOL_INIT.out.data_types))

    // Collect outputs
    ch_results = ch_results.mix(PNEUMOCAT.out.results)
    ch_logs = ch_logs.mix(PNEUMOCAT.out.logs)
    ch_nf_logs = ch_nf_logs.mix(PNEUMOCAT.out.nf_logs)
