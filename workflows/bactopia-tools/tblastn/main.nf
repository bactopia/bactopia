 ...
 * @citation blast
 *
 * @subworkflows utils_bactopia-tools, tblastn
 *
 * @input rundir
...
workflow {
    ...
    ch_tblastn = TBLASTN(ch_bactopiatool.blastdb, params.tblastn_query)

    publish:
    // Per-sample
    sample_outputs = ch_tblastn.sample_outputs
    sample_nf_logs = collectNextflowLogs(ch_tblastn.sample_outputs)
    // Run-level
    run_outputs = ch_tblastn.run_outputs
    run_nf_logs = collectNextflowLogs(ch_tblastn.run_outputs)
}
