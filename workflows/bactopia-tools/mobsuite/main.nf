...
workflow {
    ...
    BACTOPIATOOL_INIT()
    MOBSUITE(formatSamples(BACTOPIATOOL_INIT.out.samples, BACTOPIATOOL_INIT.out.data_types))

    // Collect outputs
    ch_results = ch_results.mix(MOBSUITE.out.results)
    ch_logs = ch_logs.mix(MOBSUITE.out.logs)
    ch_nf_logs = ch_nf_logs.mix(MOBSUITE.out.nf_logs)
