...
// Enteropia Specific
include { ECTYPER } from '../subworkflows/local/ectyper/main' addParams(is_main: true)
include { KLEBORATE } from '../subworkflows/local/kleborate/main' addParams(is_main: true)
include { SEQSERO2 } from '../subworkflows/local/seqsero2/main' addParams(is_main: true)
...
workflow ENTEROPIA {
    ...
    // Enteric Specific
    if (params.species.toLowerCase().startsWith("escherichia")) {
        ECTYPER(ASSEMBLE_GENOME.out.fna)
        // EZCLERMONT()
        // SEROTYPEFINDER()
        ch_versions = ch_versions.mix(ECTYPER.out.versions.first())
    } else if (params.species.toLowerCase().startsWith("klebsiella")) {
        KLEBORATE(ASSEMBLE_GENOME.out.fna)
        ch_versions = ch_versions.mix(KLEBORATE.out.versions.first())
    } else if (params.species..toLowerCase()startsWith("salmonella")) {
        SEQSERO2(ASSEMBLE_GENOME.out.fna)
        // SISTR(ASSEMBLE_GENOME.out.fna)
        // BIOHANSEL(ASSEMBLE_GENOME.out.fna, '<GENUS_NAME>')
        // GENOTYPHI(ASSEMBLE_GENOME.out.fna)
        // SEROTOOLS() https://github.com/CFSAN-Biostatistics/SeroTools
        ch_versions = ch_versions.mix(SEQSERO2.out.versions.first())
    } //else if (params.species.startsWith("shigella")) {
        // SHIGATYPER() https://github.com/CFSAN-Biostatistics/shigatyper
        // SHIGEIFINDER() https://github.com/LanLab/ShigEiFinder
        // SONNEITYPING()https://github.com/katholt/sonneityping
    //}

    // STECFINDER() https://github.com/LanLab/STECFinder
    // CPGENEPROFILER() https://github.com/ramadatta/CPgeneProfiler
    // Collect Versions
