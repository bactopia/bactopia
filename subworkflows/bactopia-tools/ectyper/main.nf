...
process ectyper {
    ...
    output:
    file("${sample}/*")
    file("${sample}/${sample}.tsv") into MERGE_ECTYPER
    ...
    verify = params.verify ? "--verify --refseq ${params.refseq}" : ''
    dbpath = params.dbpath ? "--dbpath ${params.dbpath}" : ''
    """
    if [ "!{is_gzipped}" == "true" ]; then
        zcat !{fasta} > !{sample}.fasta
    else
        ln -s `readlink !{fasta}` !{sample}.fasta
    fi

    ectyper --input !{sample}.fasta \
        --cores !{task.cpus} \
        -opid !{params.opid} \
        -hpid !{params.hpid} \
        -opcov !{params.opcov} \
        -hpcov !{params.hpcov} \
        -o !{sample}/ !{debug} !{sequence} !{verify} !{dbpath}

    mv !{sample}/output.tsv !{sample}/!{sample}.tsv
    """
}

process merge_ectyper {
    publishDir "${OUTDIR}", mode: "${params.publish_mode}", overwrite: OVERWRITE, pattern: "ectyper-results.txt"
    beforeScript 'ulimit -Ss unlimited'

    input:
    file(tsv) from MERGE_ECTYPER.collect()

    output:
    file("ectyper-results.txt")

    shell:
    """
    mkdir results/
    ls *.tsv | head -n1 | xargs -I {} head -n1 {} > results/ectyper-results.txt
    cat *.tsv | grep -v "GeneContigNames" >> results/ectyper-results.txt
    mv results/ectyper-results.txt ./
    """
}
...
def check_input_params() {
    ...
    // ECTyper parameter checks
    error += is_positive_integer(params.opid, 'opid')
    error += is_positive_integer(params.hpid, 'hpid')
    error += is_positive_integer(params.opcov, 'opcov')
    error += is_positive_integer(params.hpcov, 'hpcov')

    if (params.dbpath) {
        error += file_exists(params.dbpath, '--dbpath')
    }

    if (params.verify) {
        if (params.refseq) {
            error += file_exists(params.refseq, '--dbpath')
        } else {
            log.error("'--refseq' must also be used with '--verify'")
            error += 1
        }
    }
...
def print_help() {
    ...
    ECTyper Parameters:
        --opid INT              Percent identity required for an O antigen allele match
                                    Default ${params.opid}

        --hpid INT              Percent identity required for an H antigen allele match
                                    Default ${params.hpid}
    
        --opcov INT             Minumum percent coverage required for an O antigen allele match
                                    Default: ${params.opcov}

        --hpcov INT             Minumum percent coverage required for an H antigen allele match
                                    Default: ${params.hpcov}

        --verify                Enable E. coli species verification (requires '--refseq')

        --refseq PATH           Location of pre-computed Mash RefSeq sketch. If provided, genomes
                                    identified as non-E. coli will have their species identified 
                                    using MASH.

                                    **NOTE**
                                    If 'bactopia datasets' was used the RefSeq Mash sketch is
                                    likely available: /path/to/datasets/minmer/refseq-k21-s1000.msh
        ...
        --dbpath PATH           Path to a custom database of O and H antigen alleles in JSON format.
